  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: deps }, { data: wdrs }, { data: invs }] = await Promise.all([
        supabase
          .from("deposits")
          .select("id, amount, status, token, network, created_at")
          .eq("user_id", user.id),
        supabase
          .from("withdrawals")
          .select("id, amount, status, token, created_at")
          .eq("user_id", user.id),
        supabase
          .from("investments")
          .select("id, amount, status, daily_roi_pct, created_at, plans(name)")
          .eq("user_id", user.id),
      ]);
      const all: Row[] = [
        ...(deps || []).map((d: any) => ({
        ...
        ...(invs || []).map((i: any) => ({
          ...
          status: i.status,
          meta: `${i.plans?.name} · ${i.daily_roi_pct}%/d`,
          date: i.created_at,
        })),
