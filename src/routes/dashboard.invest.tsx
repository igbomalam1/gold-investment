    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Pick a plan, set your amount, start earning daily ROI.
        </p>
      </div>
          ...
          {plans.map((p) => (
            ...
            >
              ...
              <div className="mt-3 font-display text-3xl">
                {p.daily_roi_pct}%<span className="text-sm text-muted-foreground"> /day</span>
              </div>
              ...
              <form onSubmit={submit} className="mt-5 space-y-4">
                <div className="rounded-xl bg-background/40 p-3 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-success">+{picked.daily_roi_pct}%</span>
                  </div>
                  <div className="mt-1 flex justify-between">
                    <span className="text-muted-foreground">Duration</span>
                    <span>{picked.duration_days} days</span>
                  </div>
                  <div className="mt-1 flex justify-between">
                    <span className="text-muted-foreground">Range</span>
                    <span>
                      {formatCurrency(picked.min_amount)} — {formatCurrency(picked.max_amount)}
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <input
                    required
                    type="number"
                    min={picked.min_amount}
                    max={Math.min(Number(picked.max_amount), Number(profile?.balance ?? 0))}
                    placeholder={`${picked.min_amount}`}
                    value={amount}
                    ...
                    <div className="rounded-[calc(0.75rem-4px)] bg-card/80 p-3 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono font-semibold text-success">
                          +{formatCurrency((+amount * picked.daily_roi_pct) / 100)}
                        </span>
                      </div>
                      <div className="mt-1 flex justify-between">
                        <span className="text-muted-foreground">
                          Total ROI ({picked.duration_days}d)
                        </span>
                        <span className="font-mono font-semibold text-gradient-gold">
                          {formatCurrency(
                            ((+amount * picked.daily_roi_pct) / 100) * picked.duration_days,
                          )}
                        </span>
                      </div>
                    </div>
